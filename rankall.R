rankall <- function(routcome, num = "best") {
  ...
  if(routcome == "heart failure") {
    fstate_data <- foutcome[,c(2,7,17)]
    tst1 <- fstate_data[order(suppressWarnings(as.numeric(fstate_data[,3])),fstate_data[,2],fstate_data[,1],na.last = NA),]
    if(num == "worst") {
      num <- nrow(tst1)
    }
    return(tst1$Hospital.Name[num])
    #return(tst1)
  }
